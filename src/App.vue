<template>
  <div>
    <div>
      <UserCard :name="firstName" , :age="24" , :is-active="isActive" />
    </div>
    <div>
      <HookDemo />
    </div>
    ...
    <div class="container">
      <h1>Vue Playground - Let's Master Reactivity!</h1>
      ...
      <div class="card">
        ...
        <div v-else-if="isGuest">
          <p class="guest">👤 You are browsing as a guest</p>
        </div>

        <div v-else>
          <p class="warning">🔒 Please log in or continue as guest</p>
        </div>
      </div>
    </div>
    ...
    <!-- TASK 2: Highlight Toggle -->
    <div class="card">
      <button @click="toggleHighlight">
        {{ isHighlighted ? "Remove Highlight" : "Highlight Text" }}
      </button>
      <p :class="{ highlighted: isHighlighted }">
        This text can be highlighted using dynamic class binding.
      </p>
    </div>

    <!-- Search -->
    <div class="card">
      <input v-model="searchItem" placeholder="Search users..." class="search-input" />
    </div>

    <!-- TASK 3: Role Filter Dropdown -->
    <div class="card">
    ...
    <div class="card">
      ...
      <ul>
        <li v-for="user in filteredUsers" :key="user.id" class="user-item">
          ...
          <span v-if="user.active" class="active">● Active</span>
        </li>
      </ul>
    </div>
    <!-- Debug Info -->
    <div class="debug">
      <small>
        isLoggedIn: {{ isLoggedIn }} | isGuest: {{ isGuest }} | isHighlighted:
        {{ isHighlighted }} | searchItem: "{{ searchItem }}" | selectedRole: "{{
          selectedRole
        }}"
      </small>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      users: [
        ...
        { id: 2, name: "Mike Torres", role: "Designer", active: false },
        { id: 3, name: "Priya Patel", role: "Manager", active: true },
        { id: 4, name: "Jamal Wright", role: "Developer", active: true },
        { id: 5, name: "Elena Rodriguez", role: "Designer", active: false },
        { id: 6, name: "David Kim", role: "Manager", active: true },
      ],
      // v-model + computed filter
      searchItem: "",
      selectedRole: "", // ← Added for Task 3
      imageUrl:
  ...
  methods: {
    ...
    toggleLogin() {
      ...
      // Bonus: when logging out, also reset guest if you want
